...
```
operadum/
├── core/                      # Layer 2: the fused operadic runtime
│   ├── types.py               #   Colour, Operation, Composite, Interface, Spec
│   ├── enrichment.py          #   ResourceMonoid + 5 algebras (incl. LINEAR_TOKENS)
│   ├── persistence.py         #   SQLite backend (internal — never use directly)
│   ├── hooks.py               #   structural event registry
│   └── operad.py              #   THE Operad class (structure+store+cost+exec)
├── gate/                      # the Dual Gate
│   ├── type_engine.py         #   TYPE: realizability (Curry–Howard inhabitation)
│   └── res_engine.py          #   RES:  resource soundness + budget feasibility
├── wright/                    # Layer 3: the write path
│   ├── schema.py              #   Spec, Construction, Verdict, BuildResult
│   └── engine.py              #   WRIGHT — energy-routed Tiers 0–2
├── core/linear.py             # Phase 2: linear logic — Tensor/Lolli/OfCourse + checker
├── core/polytope.py           # Phase 3: associahedra, coherence rewrites, normal forms
├── core/prop.py               # Phase 3: PROP lift — resource-aware fork/merge & sharing
├── core/formal_coherence.py   # Phase 3: CoherenceProver (Mac Lane, confluence, conservation)
├── daedalus_core.py           # Layer 4: DAEDALUS generative search (branch & bound)
├── gate/pattern_miner.py      # Phase 3: mine & lift reusable patterns (Level-4 operad maps)
├── wright/solver.py           # WRIGHT Tier 3: resource-constrained synthesis
├── forge/                     # Layer 1: plugin host — event bus, capability DI, lifecycle
│   ├── core.py · events.py · plugin.py · plugins.py
├── agent.py                   # unified entry point: all layers via Forge
├── core/serialization.py      # Phase 5: wiring DSL (parse/emit) + JSON
├── wright/server.py           # Phase 5: MCP-style SynthesisServer
├── core/plugin_generator.py   # Phase 4: package an operad as a fresh DomainPlugin
├── domains/                   # pluggable content (colours + operations + algebra)
│   ├── base.py                #   DomainPlugin ABC + GroundTruthCase
│   ├── synthesis_design.py    #   domain 1: organic synthesis-route design (additive)
│   └── compute_pipeline.py    #   domain 2: data pipeline, peak-memory algebra (max)
├── gate/self_observer.py      # Phase 4: redundant/source/sink structural analysis
├── validation/benchmark.py    # synthesis-accuracy harness (optimum/buildable recall)
├── validation/domain_accuracy.py  # real-world accuracy on a domain + round-trip
├── bridges/
│   ├── komposos_bridge.py     #   compile a design → KOMPOSOS morphism graph
│   └── round_trip.py          #   KomposVerifier — KOMPOSOS audits the design
├── integrations/
│   └── komposos_mof.py        #   real KOMPOSOS-MOF generator/verdicts → OPERADUM assembly
└── tests/                     # laws, search, coherence, domains, round-trip, self-construction
examples/pipeline_demo.py      # Phase 1: design → build → run → compile, end to end
...
examples/domain_roundtrip_demo.py  # domain route → real KOMPOSOS verifies [AGREE]
examples/self_construction_demo.py # Phase 4: mine → lift → observe → package
examples/forge_agent_demo.py       # Layer 1 + Phase 5: Forge → Agent → DSL → server
```
...
result.construction.artifact("the quick brown fox")   # runs embed(tokenize(...)) → 4
```
...
| `OVERBUDGET` | ✓ | ✗ | A wiring exists but exceeds the resource budget. |
...
KOMPOSOS is cartesian — knowledge copies freely. A resource does not. The
...
python -m examples.pipeline_demo       # the full design→verify loop
```

## Roadmap (from the master spec)

- **Phase 0 — Foundations** ✅ operad runtime, persistence, enrichment, `realize()`.
- **Phase 1 — Synthesis MVP** ✅ WRIGHT Tiers 0–2, Dual Gate, four verdicts.
- **Phase 2 — Resources & optimal search** ✅ linear-logic typing (`⊗`/`⊸`/`!`),
...
- **Phase 3 — Higher structure** ✅ Polytope (associahedra, coherence rewrites,
...
- **Phase 4 — Self-construction** ✅ outcome learning (realizability rates),
  ...
  fresh DomainPlugin: mine → lift → package → reload).
- **Phase 5 — Integration & scale** ✅ wiring DSL (`parse_wiring`/`to_wiring_dsl`),
...
- **Layer 1 — Forge** ✅ the hot-loadable plugin host: event bus, capability DI
...
| `quantum-circuit` | additive (gates) | tree | **2×2 unitary matrices** |
...
network, and a MOF — *the math doesn't know which domain it's in* (spec §18). Run
...
`synthesize_diagram` searches DAGs against a validator — e.g. **XOR from NAND
...
1. **Engine correctness** (now) — type-safety, resource-soundness, cost
   conservation, optimality, normal-form uniqueness — proven and tested.
...
2. **Synthesis quality** (now, growing) — the benchmark harness measures
   optimum/buildable recall; richer benchmark domains land in Phase 4.
3. **Real-world design accuracy** (now, via the first domain plugin) — does a
   ...
   `python -m examples.domain_roundtrip_demo` → `[AGREE] engine=komposos`.
...
theorem here: `product(confidences) == cost_to_confidence(total cost)` — the
additive-cost → multiplicative-confidence map is an exact monoid homomorphism,
so both engines agree on the same structure. (Under non-additive monoids the map
is lossy → verdict `HOLLOW`, matching honest limitation #7.)
...
Proprietary / commercial — `LicenseRef-Proprietary-Commercial`.
© 2026 James Hawkins.
